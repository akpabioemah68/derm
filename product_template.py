...
class ProductTemplate(models.Model):
    ...
    multi_image_ids = fields.One2many('product.image', 'product_tmpl_id', string="Extra Images")
    ...
    category_ids = fields.Many2many(
        'product.category',
        string='Categories',
        help='Select multiple categories for this product'
    )
    ...
    def write(self, vals):
        ...
        if 'product_tag_ids' in vals:
                for product in self:
                    new_tags = self.env['product.tag'].browse(vals['product_tag_ids'][0][2]) if vals['product_tag_ids'] else []
                    old_tags = product.product_tag_ids
                    added_tags = new_tags - old_tags
                    
                    if added_tags:
                        _logger.info(f"New tags assigned to {product.name}: {added_tags.mapped('name')}")
                        
                        # Ensure product is synced with WooCommerce
                        if not product.woocommerce_product_id:
                            raise UserError(_("Please sync the product with WooCommerce before assigning a brand."))
                        
                        # Call external PHP endpoint to assign brand
                        for tag in added_tags:
                            ...
                            try:
                                response = requests.get("https://skinpulse.ng/assign_brand.php", params=payload, headers=headers)
                                
                                if response.status_code == 200:
                                    _logger.info(f"Successfully assigned brand '{tag.name}' to product '{product.name}'.")
                                else:
                                    _logger.error(f"Failed to assign brand '{tag.name}' to product '{product.name}': {response.text}")
                            except requests.RequestException as e:
                                _logger.error(f"Error calling assign_brand.php: {e}")
                                raise UserError(_("Error communicating with WooCommerce brand assignment API."))


        category_name = '5kskincare'
        default_category_name = 'Uncategorized'  # Default category to assign when removing from "5kskincare"
        
        # Fetch the '5kskincare' category
        skincare_category = self.env['product.category'].search([('name', '=', category_name)], limit=1)
        if not skincare_category:
            raise Warning(f"The category '{category_name}' does not exist. Please create it first.")
        
        # Fetch the default category
        default_category = self.env['product.category'].search([('name', '=', default_category_name)], limit=1)
        if not default_category:
            raise Warning(f"The default category '{default_category_name}' does not exist. Please create it first.")
        ...
        for record in self:
            ...
            if 'list_price' in vals:
                ...
                if new_price <= 5000:
                    if record.categ_id.id != skincare_category.id:
                        _logger.info(
                            "Assigning product '%s' (ID: %s) to category '%s' as price is <= 5000.",
                            record.name, record.id, category_name
                        )
                        record.write({'categ_id': skincare_category.id})
                elif new_price > 5000:
                    if record.categ_id.id == skincare_category.id:
                        _logger.info(
                            "Removing product '%s' (ID: %s) from category '%s' as price is > 5000.",
                            record.name, record.id, category_name
                        )
                        record.write({'categ_id': default_category.id})  # Assign default category
                    elif record.categ_id.id != skincare_category.id and record.categ_id:
                        _logger.info(
                            "Product '%s' (ID: %s) belongs to a different category '%s', moving to '%s'.",
                            record.name, record.id, record.categ_id.name, default_category_name
                        )
                        record.write({'categ_id': default_category.id})  # Assign default cat
