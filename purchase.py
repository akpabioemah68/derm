...
class PurchaseOrder(models.Model):
    ...
    def button_confirm(self):
        for order in self:
            ...
            self._send_email_to_admin(order)

        return True
    
    @api.model
    def _send_email_to_admin(self, order):
        # Get the admin user
        
        
        # Find or create the email template
        template = self.env.ref('purchase.email_template_po_raised', raise_if_not_found=False)
        if not template:

            body = f"""
                <p>Dear Admin,</p>
                <p>A new Purchase Order has been raised.</p>
                <p><strong>PO Number:</strong> {order.name}<br/>
                <strong>Vendor:</strong> {order.partner_id.name}<br/>
                <strong>Total:</strong> {order.currency_id.symbol}{order.amount_total}</p>
                <p>Thank you.</p>
            """
            # Fallback: create a basic email template dynamically
            template = self.env['mail.template'].create({
                'name': 'PO Raised Notification',
                'model_id': self.env['ir.model']._get('purchase.order').id,
                'subject': _('New Purchase Order Raised: %s') % order.name,
                'email_from': 'oga@skinpulse.online',
                'email_to': 'mailesom@gmail.com',
                'body_html':body,
            })
...
class PurchaseOrderLine(models.Model):
    ...
    sale_price = fields.Float(
        string="Sales Price",
        help="Expected sales price for this product when sold"
    )
